---
title: "Multivariate Analysis: Second Assignment"
...
2. MDS with $k$-means and Euclidean distance ($k=7$).
...
Comparison between clustering PCA data against MDS data using $k$-means and the euclidean distance ($k=4$ against $k=7$).
:::
...
![PCA 3D](pca.png){#fig-pca-3d}

![MDS 3D](mds.png){#fig-mds-3d}

:::{#tbl-pca}

```{r}
tabla_clusters_pca %>%
  gt::tab_options(table.width = gt::pct(90)) %>% # Force width to 100% of page
  gt::as_latex() # Ensure it outputs latex
```
